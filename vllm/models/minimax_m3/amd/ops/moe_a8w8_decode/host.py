...
Same contracts as ``moe_a16w4_decode/host.py`` (aiter ``moe_sorting`` buffers on
the sorted path, ``topk_ids`` / ``topk_weights`` on the inline-sort path); the
...
from vllm.models.minimax_m3.amd.ops.moe_a16w4_decode.host import _get, _run_compiled
