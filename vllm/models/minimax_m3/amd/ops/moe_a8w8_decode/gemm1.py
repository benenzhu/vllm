...
class _Tile:
    ...
    def __init__(self, TILE_N, KW, KB, RT, prefetch, K, INTER):
        self.TILE_N, self.KW, self.KB, self.RT, self.prefetch = TILE_N, KW, KB, RT, prefetch
        self.BM = RT * 16
        ...
        self.RS_T, self.KSLOT_T, self.SLOT_T = self.RS // 16, self.KSLOT // 16, self.SLOT // 16
...
def compile_gemm1(
    *,
    ...
    name = (
        ...
        + (f"_wide{WIDE_BM}t{WIDE_TILE[0]}k{WIDE_TILE[1]}b{WIDE_TILE[2]}" if wide else "")
    )
    ...
    def kernel(
        ...
        wr = buffer_ops.create_buffer_resource_from_addr(arg_bq, num_records_bytes=W_BYTES)
        ...
        def body(c, mbase, nb, e, ld_row, ld_tok, ep_tok, cumsum0):
            ...
            ld_gdw = [(ld_tok[rt] * (K * 2) + l16 * 16) // 4 for rt in range_constexpr(c.RT)]
            ...
            def stage_a_batch(regs, slot):
                for rt in range_constexpr(c.RT):
                    for kw in range_constexpr(c.KW):
                        for j in range_constexpr(c.KB):
                            lds_store16(
                                ld_tile[rt] + (slot * c.SLOT_T + kw * c.KSLOT_T + j * 16),
            ...
            def compute_tile(bb, sc, kt):
                ...
                if const_expr(c.RT == 1):
                    ...
                    for gu in range_constexpr(2):
                        for ni in range_constexpr(c.NI):
                            ...
                            for ku in range_constexpr(c.TPC):
                                ...
                                fx.gemm(mma_atom, acc[0][gu][ni], a_t[ku], _frag(b8), acc[0][gu][ni])
                else:
                    ...
                    for gu in range_constexpr(2):
                        ...
                        b_t.append(row)
                    for rt in range_constexpr(c.RT):
                        a_t = [_frag(read_a(kt, ku, rt)) for ku in range_constexpr(c.TPC)]
                        for gu in range_constexpr(2):
                            for ni in range_constexpr(c.NI):
                                for ku in range_constexpr(c.TPC):
                                    fx.gemm(
            ...
            for kt in range_constexpr(c.KTW):
                if const_expr(kt % c.BT == 0 and kt + c.BT < c.KTW):
                    abuf = load_a_batch(kt // c.BT + 1)  # before this iteration's W loads
                ...
                if const_expr(kt % c.BT == c.BT - 1 and kt + 1 < c.KTW):
                    stage_a_batch(abuf, (kt // c.BT + 1) % 2)  # this batch's reads are done
                    abuf = None
        ...
        if const_expr(inline_sort):
            ...
            if mb == 0:
                ...
                for iv in range(pid * (64 * NW) + tx, i32_zero_dw, narrow.NNB * 64 * NW):
            ...
            if const_expr(wide):
                ...
                if pid < n_wide_pid:
                    mbw = pid // widet.NNB
                    sorted_block(widet, mbw, mbw * WIDE_BM, pid % widet.NNB, mbw < n_wide)
                if pid >= n_wide_pid:
                    qn = pid - n_wide_pid
                    mbn = qn // narrow.NNB
                    mbasen = n_wide * WIDE_BM + mbn * BM
                    sorted_block(narrow, n_wide + mbn, mbasen, qn % narrow.NNB, mbasen < cumsum0)
            else:
                mb, nb = pid // narrow.NNB, pid % narrow.NNB
                mbase = mb * BM
