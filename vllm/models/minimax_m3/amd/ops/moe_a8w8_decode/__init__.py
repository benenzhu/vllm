...
The a16w4 decode chain (``moe_a16w4_decode``: inline-sort routing at M <= 16,
...
def _unsupported_reason(layer) -> str | None:
    ...
    if not is_mxfp8_aiter_layer(layer):
        ...
        return (
            ...
            f"{getattr(qm, 'mxfp8_backend', None)}) is not ModelOpt MXFP8 on AITER_MXFP8"
        )
...
def install_decode_fast_path(experts, prefix: str = "") -> bool:
    ...
    a16w8 kernels (same shape of hook as ``moe_a16w4_decode``). Returns True
    when installed."""
    ...
    if reason is not None:
        logger.info_once(
            "M3 FlyDSL a16w8 decode MoE not used for %s: %s", prefix or "experts", reason
        )
        return False
