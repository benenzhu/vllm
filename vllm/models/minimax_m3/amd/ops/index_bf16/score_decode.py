...
"""Decode index scorer for the bf16 index cache: row-contiguous K loads through a
...
NW = 4
# graph-constant wave budget (measured best: 4-5 blocks per wave at 800K, conc 1)
WAVES = 1024
BLK = 128
D = 128
BLOCK_BYTES = BLK * D * 2
...
MAX_REQS = 64
K_CACHE_MOD = 2  # non-temporal K loads: the cache is streamed once (+10%)


def compile_score_decode():
    name = f"m3_index_score_decode_bf16_w{WAVES}_ring{RING}"
