...
"""Prefill index scorer for the bf16 index cache (gfx950, one local index head), 4-wave
variant b: the block table entries of the next group are loaded one group ahead (loop-carried),
so a block's K loads never wait on their page id.
...
def compile_score_prefill_w4b():
    """One kernel for every shape (all sizes are runtime arguments)."""
    name = f"m3_index_score_prefill_bf16_tq{TILE_Q}_nw{NW}_s{N_SLOTS}_b" + "".join(
    ...
    def kernel(
        ...
        if (wid < n_wg) & (row0 < q_len):
            ...
            if blk0 < blk1:
                ...
                def compute_block(slot, blk):
                    ...
                    def reduce(mt):
                        ...
                        for ni in range_constexpr(NI):
                            v = acc[ni].load()
                            x = [
                                (vis[ni] >= mt * 16 + i).select(v[i], neg_inf)
                ...
                # pages of blocks g0+1 .. g0+4 are loaded one group ahead
                stage_block(load_block(page_of(blk0)), 0)
                pages = [page_of(blk0 + 1 + j) for j in range_constexpr(GROUP)]
                for gi, st in range(
                    fx.Index(0),
                    fx.Index(n_groups),
                    fx.Index(1),
                    ...
                    pages = [fx.Int32(st[j]) for j in range_constexpr(GROUP)]
                    g0 = blk0 + fx.Int32(gi) * GROUP
                    nxt_pages = [page_of(g0 + GROUP + 1 + j) for j in range_constexpr(GROUP)]
                    sc = []
                    for j in range_constexpr(GROUP):
                        blk = g0 + j
                        if const_expr(FAKE_K):
                            sc.append(compute_block(0, blk))
                        else:
                            nxt = load_block(pages[j])  # block blk+1, in flight during the MFMAs
                            sc.append(compute_block(j % N_SLOTS, blk))
                            stage_block(nxt, (j + 1) % N_SLOTS)
                    store_group(g0, sc)
                    res = yield [p.ir_value() for p in nxt_pages]
