...
"""Compile cache + launch wrappers for the bf16 indexer kernels: drop-ins for the
Triton ``minimax_m3_index_score`` / ``minimax_m3_index_topk`` /
``minimax_m3_index_decode`` of ``amd/ops/index_topk.py`` (one local index head)."""

import functools

import torch

from . import score_decode, score_prefill
...
MAX_SEGMENTS = 64
TILE_Q, GROUP = score_prefill.TILE_Q, score_prefill.GROUP
...
def index_score_prefill(
    ...
    assert index_kv_cache.is_contiguous()
    assert tuple(index_kv_cache.shape[1:]) == (128, 128)
...
def index_topk_prefill(
    ...
    if out is not None:
        ...
        topk_idx = torch.empty(
            (1, total_q, topk), dtype=torch.int32, device=score.device
        )
...
def index_score_decode(
    ...
    S = max_block rounded up to 16 (the fused Triton selector's input). The
    kernel's work split is one lane per request, so requests are scored in
    groups of 64 (one launch each)."""
    total_q, num_idx_heads, head_dim = idx_q.shape
    num_reqs = seq_lens.shape[0]
    assert num_idx_heads == 1 and head_dim == 128
    assert total_q == num_reqs * decode_query_len and 1 <= decode_query_len <= 16
    assert idx_q.dtype == torch.bfloat16 and index_kv_cache.dtype == torch.bfloat16
    assert idx_q.stride(2) == 1 and idx_q.stride(0) == head_dim
    assert index_kv_cache.is_contiguous()
    assert tuple(index_kv_cache.shape[1:]) == (128, 128)
    ...
    S = _cdiv(max_block, 16) * 16
    score = torch.empty((1, total_q, S), dtype=torch.float32, device=idx_q.device)
    for r0 in range(0, num_reqs, score_decode.MAX_REQS):
        n = min(score_decode.MAX_REQS, num_reqs - r0)
        _run_compiled(
            get_score_decode(),
            idx_q.data_ptr() + r0 * decode_query_len * head_dim * 2,
            index_kv_cache.data_ptr(),
            score.data_ptr() + r0 * decode_query_len * S * 4,
...
def index_decode(
    ...
    out=None,
    *,
    attention_block_table=None,
    sparse_block_table_out=None,
    sparse_context_lens_out=None,
    block_page_stride=None,
    ...
    if out is not None:
        ...
        topk_idx = torch.empty(
            (1, total_q, topk), dtype=torch.int32, device=idx_q.device
        )
    num_topk_chunks, single_tile_guaranteed, adaptive_final_merge = (
        up._decode_topk_launch_policy(
    ...
    if completion_counter is None:
        active_counter = torch.zeros(
            (num_idx_heads, batch), dtype=torch.int32, device=idx_q.device
        )
    else:
