...
def _atomic_bf16_epilog(
    ...
    if pre_packed is not None:
        # issued in the kernel prologue (a_direct); no post-loop round trip
        packed = pre_packed
        weight = pre_weight
    else:
...
def _gemm2_body_a16w4(
    ...
    *,
    ...
    # Split-K across CTAs (ksplit > 1): this CTA covers K tiles [k_split_idx*K_TILES,
    # +K_TILES);
    # the bf16 atomic epilogue sums the partials (same idea as CK-tile's split-K 3 at
    # M=4).
    assert K_TILES_TOTAL % ksplit == 0, (
        f"K tiles {K_TILES_TOTAL} not divisible by ksplit {ksplit}"
    )
    ...
    # W2 (raw bf16) preshuffle layout (N-major == shuffle_weight (16,16)), bf16-elem
    # units:
    ...
    # pad_mask: padding rows (sorted token id >= n_tokens) are pointed past num_records
    # so the
    # buffer unit returns zeros without an L2 request. At M=4 only 1-2 of the 16 rows of
    # an
    # m-block are real, so this removes ~15/16 of the A traffic (20 MB per call at 816
    # CTAs).
    _A_OOB_DW = 0x3FFFF000  # dword index; *4 = 0xFFFFC000 bytes, + col stays < 2^32
    x_buf = _global_i32_buffer_view(
        arg_a, fx.Int64(0xFFFFC000 if pad_mask else 0xFFFFFFFF)
    )
    ...
    # direct-to-LDS is 4 B/lane only (the 16 B form fails LLVM ISA lowering), so stage
    # via
    # VGPRs like the legacy kernel: buffer_load 16 B gmem->regs then ds_write 16 B
    # regs->
    # LDS (both b128, valid on CDNA3), preserving the same swizzled-src / linear-LDS
    # layout.
    ...
    # ---- A direct global->VGPR (a_direct): the stage-1 intermediate is already by
    # sorted
    # row, so each lane's MFMA fragment is 16 contiguous bytes at (m_row + lane%16, K
    # slice
    ...
    a_row_bytes = [
        (m_row + fx.Int32(mi * 16) + lane_mod_16) * fx.Int32(K * 2)
    ...
    if const_expr(pad_mask and a_direct):
        for mi in range_constexpr(m_repeat):
            if pre_dirtok is not None:
                ...
                _tok = fx.Int32(
                    _global_i32_at(arg_stids, m_row + fx.Int32(mi * 16) + lane_mod_16)
                )
            _ok = (_tok & fx.Int32(0x00FFFFFF)) < i32_M
            a_row_bytes[mi] = _ok.select(
                a_row_bytes[mi], fx.Int32(0xFFFFC000)
            )  # >= num_records -> OOB zero
    ...
    # One scale dword per lane covers a 32-col block x 256 K (2 bytes per 128-K half,
    # both
    # 16-col halves), so the two 128-K halves of a 256-K tile and the two 16-col n-tiles
    # of
    # a 32-col block all read the SAME dword. `_sc_cache` (trace-time dict keyed by the
    # python-constant (32-col block, 256-K group)) issues it once: 8 -> 2 gathers per
    # wave
    # at tn256/tk256 (ATT: these gathers were the top VMEM issue stall).
    _sc_cache = {}
    _share_ok = (
        scale_share and (TILE_K * K_TILES) % 256 == 0 and (_n_per_wave % 32 == 0)
    )

    def load_b_scale(base_k, mni, n_pack, ckey=None):
        # Per-lane scalar e8m0 gather (dict-cached across ku; shared across
        # tiles/n-tiles
    ...
    for ni in range_constexpr(num_acc_n):
        ...
        # bf16 W folds expert_off into the resource base (see w_tiles); mxfp4/int4 index
        # it.
...
def compile_gemm2_a16w4_port(
    ...
    *,
    ...
    ``ksplit``: split-K across CTAs (grid x ksplit; partials summed by the atomic
    epilogue).
    ...
    assert ksplit >= 1 and (ksplit == 1 or not persist), (
        "ksplit needs the non-persistent grid"
    )
    if hoist is None:
        hoist = a_direct
    assert not hoist or (xcd_swizzle <= 0 and not persist), (
        "hoist needs xcd_swizzle<=0 and not persist"
    )
    if a_direct:
    ...
    def gemm2_kernel(
        ...
        if const_expr(ksplit > 1):
            ...
            tile_i32 = bx_i32
            ks_i32 = None
        # Hoisted prologue (tile == launch index when xcd_swizzle == 0): issue the
        # expert id
        # and the epilogue's token ids / routing weights together with cumsum0 instead
        # of
        # behind the bound check and after the K loop (each is an exposed round trip at
        # M=4).
        ...
        if const_expr(hoist):
            ...
            if const_expr(pairs):
                ...
                pre_e, _owner, _, _build_tab = decode_pairs_table(
                    arg_stids, i32_M, TOPK, _mb, lane, _tab, max_pairs=max_pairs
                )
                if _owner:
                    _build_tab()
                ...
                def _sweight_at(
                    row, fused
                ):  # topk_weights[token*TOPK + slot], clamped for pads
                    _f = fx.Int32(fused)
                    _pair = (_f & fx.Int32(0x00FFFFFF)) * fx.Int32(TOPK) + (
                        _f >> fx.Int32(24)
                    )
                    _pair = fx.Int32(arith.minsi(_raw(_pair), _raw(_np_m1)))
                    return llvm.load(
                        T.f32, _gep1(_sw_base, _pair * fx.Int32(4)), invariant=True
                    )
            else:
                ...
                def _stid_at(row):
                    return llvm.load(
                        T.i32,
                        _gep1(_stids_base, (_mrow + row) * fx.Int32(4)),
                        invariant=True,
                    )
                ...
                def _sweight_at(row, fused):
                    return llvm.load(
                        T.f32,
                        _gep1(_sw_base, (_mrow + row) * fx.Int32(4)),
                        invariant=True,
                    )
            ...
            if const_expr(pad_mask):
                # token ids of the rows this thread / lane loads (same row maps as the
                # body)
                if const_expr(a_direct):
                    pre_dirtok = []
                    for mi in range_constexpr(BM // 16):
                        pre_dirtok.append(
                            fx.Int32(
                                _stid_at(fx.Int32(mi * 16) + (tx_i32 % fx.Int32(16)))
                            )
                        )
                else:
                    _tkd = (TILE_K * 2) // 4
                    ...
                    for i in range_constexpr((BM * TILE_K * 2) // (256 * 16)):
                        _rl = (
                            tx_i32 * fx.Int32(4) + fx.Int32(i * 256 * 4)
                        ) // fx.Int32(_tkd)
