...
def compile_moe_reduce_fp8(
    *,
    ...
    def kernel_reduce_fp8(OUT: fx.Tensor, OUT_sc: fx.Tensor, W: fx.Tensor, Y: fx.Tensor, n_tokens: fx.Int32):
        ...
        if tok < n_tokens:
            ...
            row0 = tok * topk
            ws = [
                fx.Float32(buffer_ops.buffer_load(w_rsrc, row0 + k, vec_width=1, dtype=fx.Float32)) for k in range(topk)
            ]
            data = [
                [
                    Vec(
            ...
            y_dw = tok * (H // 2) + seg * (CHUNK_DW * 2 * CPW) + lane * 8  # bf16 row in dwords
            for i in range_constexpr(CPW):
                packed = [_pack_bf16x2(acc[i][2 * j], acc[i][2 * j + 1]) for j in range(8)]
                base = y_dw + i * (CHUNK_DW * 2)
                buffer_ops.buffer_store(_v4i32(packed[0:4]), y_rsrc, base, cache_modifier=ST)
                buffer_ops.buffer_store(_v4i32(packed[4:8]), y_rsrc, base + 4, cache_modifier=ST)

    @flyc.jit
    def launch_reduce_fp8(OUT: fx.Tensor, OUT_sc: fx.Tensor, W: fx.Tensor, Y: fx.Tensor, n_tokens: fx.Int32, stream: fx.Stream):
