...
Chain: ``moe_a4w4_prefill.sort`` with ``block_m_for(M)``-row blocks, aiter's
...
GEMM2_MAX_TILE_M = 128  # gemm2 row tile = the sort block (64-row tiles on a 128 sort: 206 vs 194 us at 2048)
...
@functools.cache
def _get_gemm1(hidden_size: int, intermediate_size: int, num_experts: int, block_m: int):
    from .gemm1 import compile_moe_gemm1_mid

    return compile_moe_gemm1_mid(H=hidden_size, I=intermediate_size, E=num_experts, BM=block_m)


@functools.cache
def _get_gemm2(
    hidden_size: int, intermediate_size: int, num_experts: int, tile_m: int, block_m: int
):
    from .gemm2 import compile_moe_gemm2_mid

    return compile_moe_gemm2_mid(
        H=hidden_size, I=intermediate_size, E=num_experts, BM=tile_m, sort_block_m=block_m
    )
