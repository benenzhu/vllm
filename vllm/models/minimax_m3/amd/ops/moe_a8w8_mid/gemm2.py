...
``moe_a16w4_decode.gemm2._atomic_bf16_epilog`` (LDS [BM, TN] f32, aliasing
...
Layouts (bytes), those of ``moe_a8w8_prefill/gemm2.py``:
  ...
  W2       [E, H/16, I/64, 4, 16, 16]      shuffle_weight(is_guinterleave, gate_up=False)
  W2_sc    [E*H/32, I/256, 4, 16] dwords   shuffle_scale (bytes: N-half + 2 x 128-K half)
...
from vllm.models.minimax_m3.amd.ops.moe_a16w4_decode.utils import _lds_ptr3, _raw
...
PREFETCH = 3  # W K-steps in flight per wave (K = 768 is 6 steps); 2 at BM 128 (registers)
W_CACHE_MOD = 2  # non-temporal W loads
BLOCK_K = 128
LDS_PAD = 16


def compile_moe_gemm2_mid(*, H: int, I: int, E: int, BM: int, sort_block_m: int | None = None):  # noqa: E741
    ...
    def kernel_gemm2(
        ...
        if mb * BM < valid_rows:
            eid_rsrc = buffer_ops.create_buffer_resource(
                sorted_expert_ids, max_size=False, num_records_bytes=(num_m_blocks // SUB) * 4
            )
            expert = fx.Int32(
                buffer_ops.buffer_load(
            ...
            as_rsrc = buffer_ops.create_buffer_resource(
                A_scale, max_size=False, num_records_bytes=num_m_blocks * (BM * (I // 32))
            )
            ...
            def load_a_batch(b):
                return [
                    buffer_ops.buffer_load(
                        a_rsrc, a_gdw[j], vec_width=4, dtype=fx.Int32, soffset_bytes=b * ROWB
                    )
                    for j in range_constexpr(NLD)
                ]
            ...
            def stage_a_batch(regs, slot):
                for j in range_constexpr(NLD):
                    llvm.StoreOp(
                        _raw(regs[j]), _lds_ptr3(lds_base, slot * SLOT + a_lbyte[j]), alignment=16
                    )
            ...
            ws_dw = [
                ((expert * N32 + n16_base // 2 + np) * SC_BLOCKS_PER_ROW32) * 64 + q16 * 16 + l16
            ...
            def load_b_step(kt):
                bb = [
                    _pack8(
                        buffer_ops.buffer_load(
                            w_rsrc, w_dw[ni], vec_width=4, dtype=fx.Int32,
                            cache_modifier=W_CACHE_MOD, soffset_bytes=kt * 2048,
                        ),
                        buffer_ops.buffer_load(
                            w_rsrc, w_dw[ni], vec_width=4, dtype=fx.Int32,
                            cache_modifier=W_CACHE_MOD, soffset_bytes=kt * 2048 + 1024,
                        ),
                    )
                    for ni in range_constexpr(NI)
                ]
                if const_expr(kt % 2 == 0):
                    sa = [
                        buffer_ops.buffer_load(
                            as_rsrc, as_dw[mp], vec_width=1, dtype=fx.Int32,
                    ...
                    sb = [
                        buffer_ops.buffer_load(
                            ws_rsrc, ws_dw[np], vec_width=1, dtype=fx.Int32,
                            soffset_bytes=(kt // 2) * 256,
                        )
                        for np in range_constexpr(NI // 2)
                    ]
            ...
            def read_a_tile(kt):
                ...
                for mi in range_constexpr(MR):
                    ...
                    h0 = llvm.load(T.vec(4, T.i32), _lds_ptr3(lds_base, off), alignment=16)
                    h1 = llvm.load(T.vec(4, T.i32), _lds_ptr3(lds_base, off + 64), alignment=16)
                    out.append(_pack8(h0, h1))
                return out

            acc = [[Vec.filled(4, 0.0, fx.Float32) for _ in range(NI)] for _ in range(MR)]
            abuf = load_a_batch(0)
            ...
            for kt in range_constexpr(KT):
                ...
                if const_expr(kt % KB == KB - 1 and kt + 1 < KT):
                    stage_a_batch(abuf, (kt // KB + 1) % NSLOT)  # this batch's reads are done
            ...
            ep_ids = [
                fx.Int32(
                    buffer_ops.buffer_load(
                        ids_rsrc, m_base + (mr * 8) + m_lane, vec_width=1, dtype=fx.Int32
                    )
                )
            ...
            ep_weights = [
                fx.Float32(
                    buffer_ops.buffer_load(
                        sw_rsrc, m_base + (mr * 8) + m_lane, vec_width=1, dtype=fx.Float32
                    )
                )
                for mr in range_constexpr(BM // 8)
            ]
            fx.rocdl.s_waitcnt(lgkmcnt=0)
            fx.gpu.barrier()  # every wave is done reading A before the epilogue reuses the LDS
            _atomic_bf16_epilog(
                lds_base,
                acc,
                arg_out,
