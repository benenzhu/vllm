    ...
    def kernel_gemm1(
        ...
        if mb * BM < valid_rows:
            ...
            osc_rsrc = buffer_ops.create_buffer_resource(
                OUT_scale, max_size=False, num_records_bytes=num_m_blocks * (BM * (I // 32))
            )
            m_base = mb * BM
            ...
            ld_row = [wave * RPW + (j * ROWS_PER_LD) + lane // CH for j in range_constexpr(NLD)]
            ld_chunk = (lane % CH) * 16
            ld_tok = [
                fx.Int32(
                    buffer_ops.buffer_load(
            ...
            def load_a_batch(b):
                return [
                    buffer_ops.buffer_load(
                        a_rsrc, ld_gdw[j], vec_width=4, dtype=fx.Int32, soffset_bytes=b * ROWB
                    )
                    for j in range_constexpr(NLD)
                ]
            ...
            def stage_a_batch(regs, slot):
                for j in range_constexpr(NLD):
                    llvm.StoreOp(
                        _raw(regs[j]), _lds_ptr3(lds_base, slot * SLOT + ld_lbyte[j]), alignment=16
                    )
            ...
            # scale dwords: A (32-row group, 256-K block); W13 (16-col group, 256-K block)
            as_dw = [
                ((m_base // 32 + mp) * SC_BLOCKS_PER_ROW32) * 64 + q16 * 16 + l16
                for mp in range_constexpr(MR // 2)
            ]
            ws_dw = [
                ((expert * N16 + n16_base + ni) * SC_BLOCKS_PER_ROW32) * 64 + q16 * 16 + l16
                for ni in range_constexpr(NI)
            ]
            ...
            def load_b_step(kt):
                ...
                if const_expr(kt % 2 == 0):
                    ...
                    sb = [
                        buffer_ops.buffer_load(
                            ws_rsrc, ws_dw[ni], vec_width=1, dtype=fx.Int32,
                            soffset_bytes=(kt // 2) * 256,
                        )
                        for ni in range_constexpr(NI)
                    ]
                    ...
                    for ni in range_constexpr(NI):
                        for gu in range_constexpr(2):
                            acc[mi][ni][gu] = mfma._mfma_agpr(
                                aa[mi], bb[gu][ni], acc[mi][ni][gu],
                                sa[mi // 2], sb[ni], mi % 2, gu, kt % 2,
                            )
                if const_expr(kt % KB == KB - 1 and kt + 1 < KT):
                    stage_a_batch(abuf, (kt // KB + 1) % 2)  # this batch's reads are done

            # ---- epilogue: swiglu-OAI + MXFP8 quant ----
            flat = _pin_accumulators(
                [acc[mi][ni][gu] for mi in range(MR) for ni in range(NI) for gu in range(2)]
            )
            colgrp = col_base // 32
            ...
            if const_expr(NI == 1):
                ...
                for mi in range_constexpr(MR):
                    ...
                    buffer_ops.buffer_store(
                        packed, out_rsrc, row * I + (col_base + q16 * 4), offset_is_bytes=True
                    )
                for mp in range_constexpr(MR // 2):
            ...
            for mi in range_constexpr(MR if NI == 2 else 0):
                ...
                h = [
                    _swiglu_oai(fx.Float32(gv[v]), fx.Float32(uv[v])) for v in range_constexpr(4)
                ] + [
                    _swiglu_oai(fx.Float32(gw[v]), fx.Float32(uw[v])) for v in range_constexpr(4)
                ]
                amax = _maxf_nn(fx.math.absf(h[0]), fx.math.absf(h[1]))
