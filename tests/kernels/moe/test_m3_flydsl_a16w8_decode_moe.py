...
def m3_weights():
    ...
    w13 = torch.randn((NUM_EXPERTS, 2 * INTER, HIDDEN), dtype=torch.bfloat16, device=device)
...
def test_a16w8_decode_moe_graph_replay(m3_weights, m):
    ...
    device = shuffled[0].device
    inputs = [
        (torch.randn((m, HIDDEN), dtype=torch.bfloat16, device=device), *_routing(m, device))
        for _ in range(4)
    ]
